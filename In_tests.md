...
- **При повышенном напряжении (около +10-15% от 24V, т.е. примерно 26.5V):**
  - Срабатывают защитные механизмы (предохранитель + стабилитрон разрывают цепь при превышении напряжения).
...
3. **Проверка пониженного напряжения:**
   ...
   - Измерьте напряжение на ключевых точках (после конвертации на 3.3V)
...
7. Подключите тестовые нагрузки:
   - Для аналого-цифровой платы включите вентилятор на один из каналов 220VAC (через симисторную плату). В данном режиме вентилятор пока не запитан 220VAC.
...
8. Измерьте мультиметром напряжение на TP7 (24V линия) и на выходах конвертации 3.3V.
...
- Линии RTS, отдельно выделенные на каждую дочернюю плату, поднимаются до номинала при запросе дочернего контроллера и опускаются при отсутствии активности.
...
4. Измерьте уровни сигналов RTS при активном и неактивном состоянии (например, для платы симисторов, аналоговых I/O, шаговых двигателей).
...
2. На плате контроллера должна быть прошивка, запрашивающая состояния дочерних плат.
...
### **Тестовый сценарий 7: Проверка срабатывания защиты от перенапряжения и обратной полярности (этот тест можно объеденить с тестами напряжений)**
