...
5. Подать питание 24В; на управляющей плате LED1 должен мигать 10 раз в секунду
...
26. Подать питание 24В
27. Все концевики на обеих платах ШД должны быть разомкнуты (нет ничего в зазоре)
27. Все оптопары led плат должны быть разомкнуты (нет ничего в зазоре)
28. Отправить 'c' в отладочный терминал
29. LED1 на обеих ШД и на дискретной должен мигать 10 раз в секунду; проверять не менее 10 секунд
30. Отправить 'c' в отладочный терминал
31. LED1 на обеих ШД должен мигать 1 раз в секунду
32. Заклеить все концевики обеих ШД (поместить в зазор скотч) - светодиод на каждом концевике должен переключиться
...
38. Отправить 'c' в отладочный терминал
39. LED1 на управляющей плате должен мигать 10 раз в секунду
40. Не дольше чем через 45 секунд LED1 на управляющей плате должен светиться постоянно
41. Отключить питание
...
6. На всех платах LED1 должен светиться постоянно
7. Снять питание с линии 24В
...
6. По готовности, отправить "s" в UART терминал (старт теста)
7. Отключить питание печи
...
8. LED1 на всех платах должен начать мигать 10 раз в секунду
9. Проверить состояние компонентов плат (пирометром\тепловизором) - схема компонентов в графическом виде приведёна в приложении 1. Если компонент нагрет выше 50°C, пометить его на схеме.
10. LED1 на всех платах должен мигать раз в секунду
11. Подождать 10 минут, чтобы остудить спирали и платы

12. Отправить команду "s"
13. Ожидать завершения 2й итерации нагрузочного тестирования (±2 мин)
14. Проверить состояние компонентов плат (пирометром\тепловизором) - схема компонентов в графическом виде приведёна в приложении 1. Если компонент нагрет выше 50°C, пометить его на схеме.
15. LED1 на всех платах должен мигать раз в секунду
...
20. LED1 на мастер плате должен светиться постоянно
