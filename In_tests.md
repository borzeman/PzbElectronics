...
**Ожидаемые результаты:**
- Отсутствие перегрева компонентов материнской и дочерних плат (sensor_readout, проверка пирометром).
...
1. Подготовить оборудование согласно общей инструкции подключения:
   - Выполнить все шаги подключения для **материнской платы** и **платы контроллера**.
   - Выполнить шаги подключения группы 3 (Группа 1 + Группа 2).

2. Проверить наличие питания:
   - Убедиться, что на входе реле печи присутствует 230В.
   - Проверить целостность всех подключений и позвать ревьюера для проверки корректности подключения.

3. Подключить ПК к UART контроллера для мониторинга данных прошивки init_func.

4. Подать питание 24В на материнскую плату.
   - Убедиться, что init_func запускается: модули отвечают 11111111, RTS взводится, функциональные прошивки активны.

5. Проверить индикацию:
   - Убедиться, что индикаторы горят (led_light_control).

6. Поднять автомат питания печи под управлением triac_control.

7. Активировать платы в предельном режиме:
   - motor_control: установить двигатели на максимум.
   - triac_control: включить максимальный нагрев и 50%.
   - sensor_readout: мониторить температуру и ток.
   - led_light_control: установить максимальную яркость индикаторов.

8. Проверить на перегрев компонентов:
   - Использовать sensor_readout и пирометр для проверки температуры.

9. При необходимости повторить тест:
   - Проверить оставшиеся компоненты на перегрев, ток и корректную работу.

10. Отключить питание и выполнить заключительную проверку:
   - Убедиться в отсутствии видимых повреждений на платах, разъёмах и других компонентах.
